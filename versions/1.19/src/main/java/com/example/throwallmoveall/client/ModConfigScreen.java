...
/**
 * ModConfigScreen — Legacy version for Minecraft 1.19.x.
 *
 * Difference from 1.20+ version:
 *  - render() signature uses (MatrixStack, int, int, float) instead of (DrawContext, int, int, float)
 *  - Text drawing uses textRenderer.drawWithShadow(matrices, ...) directly
 *  - Background fill uses fill(matrices, ...) from Screen base class
 *
 * All business logic (key binding, mouse capture, etc.) is identical to the 1.20+ version.
 */
public class ModConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        this.throwComboButton = new ButtonWidget(
                blockLeft, y1, BIND_W, ROW_H,
        ...
        ButtonWidget throwIconBtn = new ButtonWidget(
                blockLeft + BIND_W + GAP, y1, ICON_W, ROW_H,
                Text.literal("\u2194"),
                btn -> {
                    listeningRow = 1;
        ...
        this.throwResetBtn = new ButtonWidget(
                blockLeft + BIND_W + GAP + ICON_W + GAP, y1, RESET_W, ROW_H,
                Text.literal("RESET"),
                btn -> {
        ...
        this.moveComboButton = new ButtonWidget(
                blockLeft, y2, BIND_W, ROW_H,
                Text.literal(config.getComboDisplayString(
        ...
        ButtonWidget moveIconBtn = new ButtonWidget(
                blockLeft + BIND_W + GAP, y2, ICON_W, ROW_H,
                Text.literal("\u2194"),
                btn -> {
                    listeningRow = 2;
        ...
        this.moveResetBtn = new ButtonWidget(
                blockLeft + BIND_W + GAP + ICON_W + GAP, y2, RESET_W, ROW_H,
                Text.literal("RESET"),
                btn -> {
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        // ── Title ─────────────────────────────────────────────────────────
        drawCenteredText(matrices, this.textRenderer, this.title, cx, 18, COLOR_TITLE);
        ...
        // ── Row labels ────────────────────────────────────────────────────
        int labelX = cx - 190;
        drawRowLabel(matrices, "dropAllMatching (ThrowAll):", labelX, startY);
        drawRowLabel(matrices, "moveAll (MoveAll):",           labelX, startY + ROW_STRIDE);
    ...
    private void drawRowLabel(MatrixStack matrices, String text, int x, int buttonY) {
        int textY = buttonY + (ROW_H - this.textRenderer.fontHeight) / 2 + 1;
        this.textRenderer.drawWithShadow(matrices, Text.literal(text), x, textY, COLOR_LABEL);
    }
