...
/**
 * Item-Scroller style Configuration GUI Screen for Minecraft 1.21.9+.
 */
public class ModConfigScreen extends Screen {
    ...
    // ── Mouse capture (1.21.9+ Click object) ─────────────────────────────────
    ...
    public boolean mouseClicked(Click click, boolean bl) {
        if (listeningRow != 0) {
            ...
            boolean alt   = click.hasAlt();
            boolean ctrl  = click.hasCtrl();
    ...
    // ── Keyboard capture (1.21.9+ KeyInput object) ───────────────────────────
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        try {
            ...
            // In MC 1.21.6+, BlurManager enforces a strict single blur per frame limit
        }
